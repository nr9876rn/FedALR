        ...
        for i in range(n_parties):
            ...
            for j in range(skew_class):
                ...
                net_dataidx_map[i] += label_idx[label][
                                      sample_num_per_client * client_cluster_id: sample_num_per_client * (
                                                  1 + client_cluster_id)]
...
def record_net_data_stats(y_train, net_dataidx_map):
    ...
    n = net_cls_counts_npy.astype(int).shape[0]
    data_info = net_cls_counts_npy.astype(int)
...
def calculate_js_divergence_matrix(data):
    """
    计算客户端标签分布之间的 JS 散度矩阵
    :param data: 客户端标签分布数据
    :return: JS 散度矩阵
    """
